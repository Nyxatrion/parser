...
def get_content(html):
    soup = BeautifulSoup(html, 'html.parser')
    items = soup.find_all('a', class_='col-sm-12')

    departments=[]
    for item in items:
        departments.append({
        'title': item.find('span', class_='h4').get_text(strip=True),
        'total_amount': item.find('span', class_='h1').get_text(strip=True),
        'link': HOST + item.find('div', class_='btn').get('href'),
        })
...
def save_file(items,path):
    with open(path,'w', newline='') as file:
        ...
        writer.writerow(['Title', 'Total Amount', 'Link'])
        for item in items:
            writer.writerow([
                item['title'],
                item['total_amount'],
...
def parse():
    ...
    if html.status_code == 200:
        ...
        for page in range(1, pages_count + 1):
            ...
            departments_2.extend(get_content(html.text))
        save_file(departments_2, FILE)
        print(f'We get {len(departments_2)} departments. ')
        os.startfile(FILE)
    else:
